...
interface NewsDao {
    ...
    @Query("SELECT * FROM news_articles WHERE category = 'Local \uD83D\uDCCD' AND country = :city ORDER BY timestamp DESC")
    fun getLocalNews(city: String): Flow<List<NewsEntity>>
    ...
    @Query("DELETE FROM news_articles WHERE category = 'Local \uD83D\uDCCD' AND country = :city")
    suspend fun clearLocalNews(city: String)
}
